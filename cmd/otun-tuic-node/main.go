// Command otun-tuic-node runs an OTun TUIC egress node: it registers on an
// OTun-S rendezvous, answers hole punches, and serves TUIC over the punched
// hole, exiting proxied traffic out this host's IP.
//
// This is the egress counterpart that stock sing-box cannot provide (its TUIC
// has no realm field). Used to validate TUIC-over-realm on real infrastructure.
//
// Config is JSON:
//
//	{
...
func main() {
	...
	log.Printf("OTun TUIC egress node started: realm=%s server=%s local=%s",
...
func systemResolver(ctx context.Context, host string, ipv4, ipv6 bool) ([]netip.Addr, error) {
	// STUN servers are configured as IPs (recommended), so this is mostly a
	// pass-through; still resolve hostnames via the system resolver as fallback.
