...
type Options struct {
	...
	// egress, so billing/kick/snapshot cover all six in one call). When nil the
	// node builds its own (back-compat single-node behavior). Only the meter is
	// shared; usermap stays per-node (each protocol engine's index space).
...
func (h egressHandler) NewConnectionEx(ctx context.Context, conn net.Conn, source M.Socksaddr, destination M.Socksaddr, onClose N.CloseHandlerFunc) {
	go func() {
		...
		if uuid, ok := h.node.uuidFor(ctx); ok {
			conn = h.node.meter.Track(uuid, conn, meter.ConnMeta{Destination: destination.String(), Protocol: "hysteria2"})
		}
...
func (h egressHandler) NewPacketConnectionEx(ctx context.Context, conn N.PacketConn, source M.Socksaddr, destination M.Socksaddr, onClose N.CloseHandlerFunc) {
	go func() {
		...
		if uuid, ok := h.node.uuidFor(ctx); ok {
			conn = h.node.meter.TrackPacket(uuid, conn, meter.ConnMeta{Destination: destination.String(), Protocol: "hysteria2"})
		}
